...
class OmbraApp(ctk.CTk):
    ...
    def _set_window_icon(self):
        """Set the window/taskbar icon. Uses icon.ico from bundle or script directory."""
    ...
    def _load_config_async(self):
        """Load config from disk in background; apply on main thread."""
        try:
            if CONFIG_FILE.exists():
                ...
                self.after(0, lambda: self.save_config())
                return
    ...
    def load_config(self):
        """Synchronous load (e.g. after Save in Settings)."""
        if CONFIG_FILE.exists():
            try:
                with open(CONFIG_FILE, "r") as f:
                    config = json.load(f)
